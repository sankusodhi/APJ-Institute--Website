    ...
    <div className="bg-white min-h-screen text-slate-800 overflow-x-hidden selection:bg-cyan-500/30">
      ...
      <section className="relative min-h-[70vh] flex flex-col justify-center items-center py-32 overflow-hidden bg-slate-50 border-b border-slate-200">
        ...
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-gradient-to-b from-blue-100/50 via-transparent to-slate-50"></div>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 text-center mt-12">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-cyan-400/30 bg-cyan-500/10 backdrop-blur-md mb-8 shadow-[0_0_20px_rgba(6,182,212,0.3)]"
          >
            <SparklesIcon className="text-cyan-400 w-4 h-4" />
            <span className="text-xs font-bold uppercase tracking-widest text-cyan-300">Admissions Open 2026-27</span>
          </motion.div>
          ...
          >
            <a href="#courses" className="px-8 py-4 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 text-slate-900 font-bold text-lg shadow-[0_0_30px_rgba(6,182,212,0.4)] hover:shadow-[0_0_50px_rgba(6,182,212,0.6)] hover:scale-105 transition-all duration-300">
              Explore Courses
            </a>
            <Link to="/contact" className="px-8 py-4 rounded-full border border-slate-300 bg-white/50 backdrop-blur-md text-slate-800 font-bold text-lg hover:bg-slate-100 transition-all duration-300 flex items-center gap-2">
      ...
      <section className="relative z-20 -mt-16 mx-auto max-w-6xl px-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-white/90 backdrop-blur-xl rounded-2xl border border-slate-200 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          {[
            { label: "Students", value: "5000+", icon: <Users className="text-cyan-400 mb-2" size={28} /> },
            { label: "Placement", value: "95%", icon: <Trophy className="text-blue-400 mb-2" size={28} /> },
            { label: "Experience", value: "10+ Yrs", icon: <Award className="text-indigo-400 mb-2" size={28} /> },
            { label: "Modern Labs", value: "15+", icon: <Microscope className="text-purple-400 mb-2" size={28} /> }
          ].map((stat, idx) => (
            <motion.div 
              key={idx}
            ...
            >
              <div className="group-hover:-translate-y-1 transition-transform duration-300">{stat.icon}</div>
              <h3 className="text-3xl font-black text-slate-900 tracking-wider my-1 drop-shadow-md">{stat.value}</h3>
          ...
          <AnimatePresence mode="popLayout">
            {filteredCourses.map((course) => (
              <motion.article
                ...
                key={course.id}
                className="group relative flex flex-col overflow-hidden rounded-3xl bg-white backdrop-blur-xl border border-slate-200 hover:border-cyan-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(6,182,212,0.15)]"
              >
                {/* Image Section */}
      ...
      <section className="bg-white py-24 relative overflow-hidden border-t border-slate-100">
        ...
        <div className="mx-auto max-w-7xl px-4 lg:px-8 relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { title: "100% Practical Training", desc: "Hands-on experience in modern simulated labs and tie-up hospitals.", icon: <ActivitySquare size={32} /> },
              { title: "Assured Placement", desc: "Dedicated placement cell with tie-ups to 50+ leading hospitals and diagnostic centers.", icon: <Building size={32} /> },
              { title: "Experienced Faculty", desc: "Learn directly from practicing doctors, surgeons, and expert technologists.", icon: <GraduationCap size={32} /> },
              { title: "Modern Medical Labs", desc: "Fully equipped pathology, microbiology, and radiology practical labs.", icon: <Microscope size={32} /> },
              { title: "Hostel Facility", desc: "Safe, secure, and comfortable separate hostels for boys and girls.", icon: <CheckCircle2 size={32} /> },
              { title: "Govt. Approved", desc: "All courses are fully recognized and affiliated with medical boards.", icon: <Star size={32} /> },
            ].map((feature, i) => (
              <motion.div 
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="bg-slate-50 border border-slate-200 p-8 rounded-3xl hover:bg-white hover:shadow-xl transition-colors duration-300 flex flex-col gap-4"
              >
      ...
      <section className="py-24 bg-slate-50 relative border-t border-slate-200">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { name: "Rahul Sharma", course: "BMLT Batch 2022", role: "Lab Tech, Apollo", text: "The practical training at APJ Institute gave me the confidence to handle advanced pathology equipment from day one of my job." },
              { name: "Priya Patel", course: "B.Sc Nursing", role: "Staff Nurse", text: "Excellent faculty and continuous hospital visits ensured I was completely ready for the challenging environment of an ICU." },
              { name: "Amit Kumar", course: "X-Ray Technician", role: "Radiologist Asst.", text: "Thanks to the placement cell, I got an offer letter before I even finished my final semester exams!" }
            ].map((review, i) => (
              <div key={i} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm relative">
                <div className="flex text-yellow-400 mb-6 gap-1">
                  {[...Array(5)].map((_, j) => <Star key={j} size={18} fill="currentColor" />)}
                </div>
                <p className="text-slate-600 text-lg mb-8 leading-relaxed">"{review.text}"</p>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-xl font-bold text-slate-900 border border-slate-300">
      ...
      <section className="relative py-24 mx-4 sm:mx-8 lg:mx-auto max-w-7xl mb-24 overflow-hidden rounded-[3rem] shadow-[0_0_100px_rgba(37,99,235,0.2)]">
        ...
        <div className="relative z-10 text-center px-4 md:px-12 py-16 flex flex-col items-center">
          ...
          <div className="flex flex-col sm:flex-row gap-5">
            <Link to="/contact" className="px-10 py-5 rounded-full bg-cyan-500 hover:bg-cyan-400 text-slate-900 font-black text-lg shadow-[0_0_40px_rgba(6,182,212,0.6)] hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2">
              Apply Now <ArrowRight size={20} />
            </Link>
            <a href="https://wa.me/919243758191" target="_blank" rel="noreferrer" className="px-10 py-5 rounded-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-black text-lg shadow-[0_0_40px_rgba(37,211,102,0.4)] hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2">
              Chat on WhatsApp
            </a>
          </div>
        </div>
